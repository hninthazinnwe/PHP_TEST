...
$result=mysqli_query($conn,"select * from location where status<>4");
if(isset($_GET['edit'])){
$id=$_GET['edit'];
$supresult=mysqli_query($conn,"select * from location where syskey=$id and status<>4");
...
<div class="wrapper">
  ...
  <div class="content-wrapper">
      ...
      <div class="col-lg-4 col-12" style="float: left;">
        <div class="box">
          ...
          <div class="box-body no-padding">
            <!-- Tab panes -->
                <form>
                <input type="hidden" name="locid" id="locid" value="<?php echo $id; ?>">
                <div class="divpad">
                  <label class="mr-25">location Code/ဆိုင္အမွတ္</label>
                  <?php if($isupdate==false): ?>
                  <input class="form-control" type="text"  name="loccode" id="loccode" value="<?php echo $code; ?>">
                  <?php else: ?>
                  <input class="form-control" type="text" disabled="disabled" name="loccode" id="loccode" value="<?php echo $code; ?>">
                  <?php endif ?> 
                </div>
                <div class="divpad">
                  <label class="mr-25">location Name/ဆိုင္အမည္</label>
                  <input class="form-control" type="text"  name="locdes" id="locdes" value="<?php echo $description; ?>"></div>
                  <div class="divpad">
                  <label class="mr-25">Phone/ဖုန္း</label>
                  <input class="form-control" type="text"  name="locphone" id="locphone" value="<?php echo $phone; ?>"></div>
                  <div class="divpad">
                  <label class="mr-25">Address/လိပ္စာ</label>
                  <input class="form-control" type="text"  name="locaddress" id="locaddress" value="<?php echo $address; ?>"></div>  
                <!--<div class="divpad">
                  <input class="btn btn-block p-10 no-border btn-info" type="file" name="file">
                </div>
                <div class="attimg">
    ...
    <?php while($row = mysqli_fetch_assoc($result)):  ?>
      ...
      <td><a class="fa fa-edit" id="edit" href="location.php?edit=<?php echo $row['syskey'] ?>" >Edit</a>
      <a class="fa fa-times" href="../dao/setup.php?locdel=<?php echo $row['syskey']; ?>">Delete</a></td>
    </tr>
    <?php $i++;endwhile; ?>
...
<!-- ./wrapper -->   
  ...
  <script>
      ...
      var syskey = $('#locid').val();//$('#type option:selected').val();
      var code = $('#loccode').val();
      var name = $('#locdes').val();
      var phone = $('#locphone').val();
      var address = $("#locaddress").val();
      var text = $(this).val();
      var info="";
      var info1={syskey:syskey,code:code,name:name,phone:phone,address:address};
      var jsoninfo = "";
