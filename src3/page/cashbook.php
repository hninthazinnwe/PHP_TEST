...
<div class="content-wrapper">
      ...
      <div class="box">
        ...
        <div class="box-body" id="content">
          ...
          <div class="row"> 
            ...
            <table class="table table-bordered table-hover" role="grid" id="salegrid">
              <thead>
                <tr class="bg-dark">
                  ...
                  <th class="text-lighter">ေဘာက္ခ်ာနံပါတ္</th>
                  <th class="text-lighter">က်ုယ္စားလွယ္အမည္</th>                  
                  <th class="text-lighter">ေငြ</th>
                </tr>
              </thead>
            </table>
          </div>
            ...
            <div class="col-lg-6">
              <label>Buy/CashPay</label>
            <div class="table-responsive">
            <table class="table table-bordered table-hover" role="grid" id="buygrid">
              <thead>
                <tr class="bg-dark">
                  <th class="text-lighter">စဥ္</th>
                  <th class="text-lighter">ေန့စြဲ</th>
           ...
           <div class="row">             
               <div class="col-lg-2">Cash On Hand : </div><div class="col-lg-3"><input class="form-control" type="text" disabled="disabled" id="cashonhand" value="0"></div>
          </div>
        </div>  
      </div>
    </section>
  ...
  <script language="javascript">
    $('#btnsearch').on('click',function(){
      ...
      method:"post",
      data:{cashbook:'',fdate:fdate,tdate:tdate,times:times,location:location},
      success:function(data){

          var header = JSON.parse(data);
          var str = "";var str1 = "";$k=1,$j=1;$saletotal= 0;$buytotal=0;
          for(i = 0; i < header[0].length; i++){
            
              if(header[0][i].code == 1 || header[0][i].code == 2)  
              {  
                var amount;
                  if(header[0][i].amount == null)
                  {
                    amount=0;
                  }
                  else amount=header[0][i].amount;

                  str += '<tr class="bg-pale-dark">\n\
                  <td>' + $k + '</td>\n\
                  <td>' + header[0][i].date + '</td>\n\
                  <td>' + header[0][i].voucher + '</td>\n\
                  <td>' + header[0][i].customer + '</td>\n\
                  <td>' + amount + '</td>'; 
                  $k++;  $saletotal += parseFloat(amount);     
              }
              else
              {
                  var amount;
                  ...
                  str1 += '<tr class="bg-pale-dark">\n\
                  <td>' + $j + '</td>\n\
                  <td>' + header[0][i].date + '</td>\n\
                  <td>' + header[0][i].voucher + '</td>\n\
                  <td>' + header[0][i].customer + '</td>\n\
                  <td>' + amount + '</td>';
                  $j++; $buytotal += parseFloat(amount); 
              }
          }
          str += '<tr class="bg-pale-dark">\n\
                  <td colspan="4">Total </td><td>' + $saletotal + '</td>'; 

          str1 += '<tr class="bg-pale-dark">\n\
                  <td colspan="4">Total </td><td>' + $buytotal + '</td>'; 

          $('#cashonhand').val($saletotal-$buytotal);                
  ...
  { 
    var disp_setting="toolbar=yes,location=no,directories=yes,menubar=yes,"; 
      disp_setting+="scrollbars=yes,width=1000, height=800, left=10, top=25"; 
    var content_vlue = document.getElementById("content").innerHTML; 
  
    var docprint=window.open("","",disp_setting); 
    docprint.document.open(); 
    docprint.document.write('</head><body onLoad="self.print()" style="width: 1000; font-size:11px; font-family:arial; font-weight:normal;">');          
