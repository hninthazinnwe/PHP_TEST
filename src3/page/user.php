...
$result=mysqli_query($conn,"SELECT u.syskey,u.userName, u.password, s.name FROM user u INNER JOIN staff s ON s.syskey = u.staffId WHERE u.status <> 4 GROUP BY u.userName, u.password, s.name");
$staff=mysqli_query($conn,"select * from staff where status <>4 ");
$location=mysqli_query($conn,"select * from location where status <>4");
if(isset($_GET['edit'])){
$id=$_GET['edit'];
$supresult=mysqli_query($conn,"SELECT u.syskey,u.userName, u.password, s.name FROM user u INNER JOIN staff s ON s.syskey = u.staffId WHERE u.status <> 4 GROUP AND s.syskey = '$id' GROUP BY u.userName, u.password, s.name");
...
<div class="wrapper">
  ...
  <div class="content-wrapper">
        ...
        <div class="box">
          ...
          <div class="box-body no-padding">
            <!-- Tab panes -->
                ...
                <div class="divpad">
                  <label class="mr-25">User Name</label>
                  <input class="form-control" type="text"  name="uname" id="uname" value="<?php echo $login; ?>">
                </div>
                <div class="divpad">
                  <label class="mr-25">Password</label>
                  <input class="form-control" type="text"  name="password" id="password" value="<?php echo $password; ?>"></div>
                <div class="divpad">
                  <label class="mr-25">Staff</label>
                  <select class="form-control" id="staff">
                    <?php while($row = mysqli_fetch_assoc($staff)):  ?>
                    <option value="<?php echo $row['syskey']; ?>"<?php if($row['syskey']==$staffid) echo 'selected="selected"'; ?>><?php echo $row['name']; ?></option>
                    <?php endwhile; ?>s
                  </select></div> 
                  ...
                  <div class="col-lg-5">
                    <div class="divpad">
                  <input class="btn btn-block p-10 no-border btn-info" type="button" name="btnsave" id="btnsave" value="Save" aria-expanded="false"></div></div>
                  <div class="col-lg-5">
                    <div class="divpad">
                  <input class="btn btn-block p-10 no-border btn-info" type="button" name="btnnew" id="btnnew" value="New" aria-expanded="false"></div></div>
                </div>
               <?php else: ?>
                  <div class="divpad">
                  <input class="btn btn-block p-10 no-border btn-info" type="button" name="btnsave" id="btnsave" value="Update" aria-expanded="false">
                </div>
                <?php endif ?>
            ...
            <table class="table table-bordered table-hover" role="grid" id="stafflist">
              <thead>
                <tr class="bg-dark">
                  ...
                  <th class="text-lighter">Staff</th>
                  <th class="text-lighter">LoginCode</th>
                  <th class="text-lighter">Password</th>
    ...
    <?php while($row = mysqli_fetch_assoc($result)):  ?>
      ...
      <td><a class="fa fa-edit" id="edit" href="user.php?edit=<?php echo $row['syskey'] ?>" >Edit</a>
      <a class="fa fa-times" href="../dao/setup.php?usrdel=<?php echo $row['syskey']; ?>">Delete</a></td>
    </tr>
    <?php $i++;endwhile; ?>
...
<!-- ./wrapper -->
  ...
  <script>
    ...
    $("#btnsave").click(function(){  
      ...
      var text = $(this).val();
      var info="";
      var info1={syskey:syskey,code:code,password:password,staff:staff};
      var jsoninfo = "";
