...
<div class="content-wrapper">
    ...
    <section class="content-header">
      ...
      <ol class="breadcrumb">
        ...
        <li class="breadcrumb-item active">ဆုမဲလက္က်န္စာရင္း</li>
      </ol>
    </section>
      ...
      <div class="box">
        <div class="box-header with-border">
          <div class="row">
            ...
            <div class="col-lg-1">
              <label>  .</label>
              <button class="btn btn-block btn-lg btn-success" id="btnreport"><i class="fa fa-print"></i></button>
            </div>
            </div>
        </div>
        <div class="clear"></div>
            ...
            <table class="table table-bordered table-hover" role="grid" id="salegrid">
              <thead>
                <tr class="bg-dark">
                  ...
                  <th class="text-lighter">ဆုမဲကုတ္</th>
                  <th class="text-lighter">ဆုမဲအမ်ိဳးအစား</th> 
                  <th class="text-lighter">Opening</th>                  
                  <th class="text-lighter">အ၀ယ္</th>
                  <th class="text-lighter">Transfer(In)</th>
                  <th class="text-lighter">အေရာင္း</th>
                  <th class="text-lighter">Transfer(Out)</th>
                  <th class="text-lighter">Closing</th>
                </tr>
              </thead>
            </table>
            </div>
...
<!-- Control Sidebar -->
  ...
  <script language="javascript">
    ...
    $('#btnsearch').on('click',function(){
      ...
      method:"post",
      data:{stockbalance:'',fdate:fdate,tdate:tdate,times:times,location:location},
      success:function(data){
          var header = JSON.parse(data);
          var str = "";var closing= 0;
          for(var i = 0; i < header[0].length; i++){
              str += '<tr class="bg-pale-dark">\n\
                  <td>' + i + '</td>\n\
                  <td>' + header[0][i].code + '</td>\n\
                  <td>' + header[0][i].description + '</td>\n\
                  <td>' + header[0][i].Oqty + '</td>\n\
                  <td>' + header[0][i].Bqty + '</td>\n\
                  <td>' + header[0][i].TIqty + '</td>\n\
                  <td>' + header[0][i].Sqty + '</td>\n\
                  <td>' + header[0][i].TOqty + '</td>\n\
                  <td>' + ((parseFloat(header[0][i].Oqty)+parseFloat(header[0][i].Bqty)+parseFloat(header[0][i].TIqty))-(parseFloat(header[0][i].TOqty)+parseFloat(header[0][i].Sqty))) + '</td></tr>'; 
                  closing += (parseFloat(header[0][i].Oqty)+parseFloat(header[0][i].Bqty)+parseFloat(header[0][i].TIqty))-(parseFloat(header[0][i].TOqty)+parseFloat(header[0][i].Sqty)); 
          }
          str += '<tr class="bg-pale-dark">\n\
                  <td colspan="8">Total Amount</td><td>' + closing + '</td>';      
          $("#salegrid").append(str);
        }
      });
    });
    ...
    $('#btnreport').on('click',function(){
      ...
      url:"../dao/cashbook.php",
      method:"post",
      data:{serialbalance:'',fdate:fdate,tdate:tdate,times:times,location:location},
      success:function(data){
          var header = JSON.parse(data);
          var str = "";var closing= 0;
          for(var i = 0; i < header[0].length; i++){
              str+="<div class='box'><div class='box-header'><div class='box-title'>"+header[0][i].description+"</div></div>";
              str+="<div class='box-body'><label>"+header[0][i].serial+"</label></div></div></div>";
          }     
          $("#group").append(str);
          Clickheretoprint();
        }
  ...
  { 
    var disp_setting="toolbar=yes,location=no,directories=yes,menubar=yes,"; 
        disp_setting+="scrollbars=yes,width=500, height=1000, left=10, top=25"; 
    var content_vlue = document.getElementById("group").innerHTML; 
  
    var docprint=window.open("","",disp_setting); 
        docprint.document.open(); 
        docprint.document.write('</head><body onLoad="self.print()" style="width: 500; font-size:11px; font-family:arial; font-weight:normal;">');          
